...
# imposes market share restrictions to avoid extreme values etc
...
function DGP_t(t::Int64, R::String, GP::global_param, global_seed::Int64, full::Bool)
    ...
    while true
        try
            ...
            RS_icm_temp = [RS_icm(c,i,P[:,:,m],exp_ξ[:,:,m],P0[m],exp_ξ0[m], σ) for c in 1:C, i in 1:N] # C*N array
            ...
            # # Only for C=2 N=1 case, ensure the market share of first firm in third markets (used in counterfactuals) is not too small (35%)
            # if C==2 && N==1
            #    @assert RS_icm_temp[1,1] > 0.35
            # end
            @assert RS0_temp < 0.25
