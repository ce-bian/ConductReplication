...
function bootstrap_MC(GP::global_param, B::Int64, T::Int64, global_seed::Int64; full = false)
    """
    Monte-carlo simulation, returns hausman stats etc
    Input:
        - GP: global parameters
        - global_seed: two random seeds
        - T: number of simulations
    ...
    """
    result = ThreadsX.map(t->bootstrap_t("B", t, GP, B, global_seed, full),1:T)
    dfB = reduce(vcat,result,cols=:union)
    result = ThreadsX.map(t->bootstrap_t("C", t, GP, B, global_seed, full),1:T)
    dfC = reduce(vcat,result,cols=:union)
    return dfB, dfC
end
