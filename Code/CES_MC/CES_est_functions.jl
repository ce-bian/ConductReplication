# using Parameters, Optim, ForwardDiff, LinearAlgebra, Distributions, Random, DataFrames, ThreadsX, NLsolve, NLSolversBase
...
function fixest_OLS(DT::DataFrame)
    """
    demand estimation - OLS regression using fixest package
    Input: 
        - DT: a dataframe for regression
    Output:
        - σ: estimated σ from fixest package
    """
    R"""
    library(data.table)
    library(fixest)
    df = as.data.table($DT)
    res <- feols(log(R_normalized) ~ log(P), data=df)
    sigma = 1 - res$coefficients[2]
    """
    σ_fixest = @rget sigma
    return σ_fixest
end
...
function est_demand_IV_se(DT::DataFrame)
    """
    IV regression, with τ as instruments - Demand estimation
    Input:
        - DT: a dataframe
    ...
    zr = Z .* res  
    avar = inv(Q'*W*Q)* Q'*W*(zr'*zr)*W*Q*inv(Q'*W*Q) 
    se = sqrt.(diag(avar))
    return 1-coef[1], se[1]
end


function fixest_IV(DT::DataFrame)
    """
    demand estimation - IV regression using fixest package
    Input: 
        - DT: a dataframe for regression
    Output:
        - σ: estimated σ from fixest package
    ...
    library(fixest)
    df = as.data.table($DT)
    res <- feols(log(R_normalized) ~ 1|log(P)~log(tau), data=df)
    sigma = 1 - res$coefficients[2]
    #print(summary(res))
    """
    σ_fixest = @rget sigma
...
function fixest_OLS_S(DT::DataFrame)
    """
    supply estimation - OLS regression using fixest package
    Input: 
        - DT: a dataframe for regression
    Output:
        - σ: estimated σ from fixest package
    ...
    library(fixest)
    df = as.data.table($DT)
    res <- feols(log(R_normalized) ~ log(tau), data=df)
    sigma = 1 - res$coefficients[2]
    """
    σ_fixest = @rget sigma
    return σ_fixest
...
function est_demand_GMM_twostep(GP::global_param, DT::DataFrame; seTRUE=true)
    """
    Nonlinear GMM  - demand estimation. 
    moment condition: E[ln(τ) ξ_true] = 0
    Input:
       - GP: global parameters (only need to pass σ)
       - DT: a dataframe
    ...
    p = DT[!, :P]
    τ = DT[!, :τ]
    τ1 = [ones(size(DT,1)) log.(τ)]
    # set up moment condition
    g = (y, τ1, p, σ) -> τ1 * (y -σ[2]-(1 - σ[1]) * log(p))
    function sample_moment(σ_est)
        """
        calculate the sample moment based on σ_est
        """
        V = mean(g(Y[i], τ1[i, :], p[i], σ_est)' for i in 1:n) 
        return V
    end

    function sample_moment_matrix(σ_est)
        #μ = μ_array
        V = hcat((g(Y[i], τ1[i, :], p[i], σ_est) for i in 1:n)...)  # Use generator instead of temporary array
        return V
    end
    function obj(ι_est, W)
        """
        objective function for nonlinear-GMM
        """
        σ_est = ι_est
        σ_est[1] = 1+exp(ι_est[1]) # ensure σ>1 -> μ>0
        V = sample_moment(σ_est)
        return V * W * V'
    end
        ...
        result = optimize(ι -> obj(ι, W1), ι_est_init, LBFGS(), options, autodiff=:forward)
        ...
        V = sample_moment_matrix(σ_est)
        # Compute the optimal weighting matrix
        W2 = inv(V * V' / n)
        # Re-estimate the parameters using the optimal weighting matrix
        result = optimize(ι -> obj(ι, W2), ι_est_init, LBFGS(),options, autodiff=:forward) # need further investigation
        ...
        if seTRUE
            ...
            v = map(i -> ForwardDiff.jacobian(θ -> g(Y[i], τ1[i, :], p[i], θ), σ_optimal), 1:n)
            dg = sum([v[i] for i in 1:n])
        
            V = sample_moment_matrix(σ_optimal)
            W = inv(V * V')
...
function est_supply_demand_GMM_twostep_modified(GP::global_param, μR_icm_R::F1, μR_0mm_R::F2, DT::DataFrame; market_list_input=missing, seTRUE= true) where {F1 <: Function, F2 <: Function}
    """
    Two-step Nonlinear GMM - Supply and demand, using LBFGS()
    two moment conditions: (1) E[ln(τ) v_true] = 0 (2) E[ln(τ) ξ_true] = 0
    Input:
    ...
    V = sample_moment_matrix(RS, RS0_long, σ_est)
    W2 = inv(V * V' / n)
    result = optimize(ι -> obj(ι, RS, RS0_long, W2), [log(σ_true-1) 0.0 0.0], LBFGS(),options, autodiff=:forward) # investigate later, why here ι_est doesn't work (sometimes it does, sometimes it doesn't, give σ_optimal = 1.0 which means ι is negative infinity)
    ι_optimal = Optim.minimizer(result)
    σ_optimal = ι_optimal
    σ_optimal[1] = 1+exp(ι_optimal[1])
