...
function Cournot_optimal_s_m_c_f(IDT::input_dt_m, c::Int) # return a scalar, optimal s for production origin c
   ...
   # C_list = [i for i in 1:C if i != c] 
    ...
    sum([
        Q_m[c, i] * (
            sum([dp_icm_dq_jkm_3[i, k, j] * dq_jkm_ds_cm_2[k, j] for i in 1:N, k in 1:C, j in 1:N]) -
            sum([dp_icm_dq_jkm_3[i, c, i] * dq_jkm_ds_cm_2[c, i] for i in 1:N]) +
            sum([dp_icm_dq_icm_1[i] * dq_icm_ds_cm_1[i] for i in 1:N]) +
            sum([dp_icm_dq_0mm[i] * dq_0mm_ds_cm for i in 1:N])
        ) for i in 1:N
    ])
)
...
function Bertrand_optimal_s_m_c_f(IDT::input_dt_m, c::Int) # return a scalar, optimal s for production origin c
   ...
   # C_list = [i for i in 1:C if i != c] 
    ...
    sum([
        P_m[c, i] * (
            sum([dq_icm_dp_jkm_3[i, k, j] * dp_jkm_ds_cm_2[k, j] for i in 1:N, k in 1:C, j in 1:N]) -
            sum([dq_icm_dp_jkm_3[i, c, i] * dp_jkm_ds_cm_2[c, i] for i in 1:N]) +
            sum([dq_icm_dp_icm_1[i] * dp_icm_ds_cm_1[i] for i in 1:N]) +
            sum([dq_icm_dp_0mm[i] * dp_0mm_ds_cm for i in 1:N])
        ) for i in 1:N
    ])
)
