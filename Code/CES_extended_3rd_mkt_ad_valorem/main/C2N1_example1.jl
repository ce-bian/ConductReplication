# correspond to the case where OG share < 25%, normal case
...
Y_m = exp(1.0)
exp_ξ_m =reshape([exp(0.6); exp(1.4)], :, 1)
...
p =plot(DTC2.S2*100, [DTC2.delta_W2*100, DTB2.delta_W2*100], 
     xlabel="", ylabel="", legend=false, framestyle = :origin, ylim = (-0.35*100, 0.1*100),
     grid = false)
...
plot!(p, x, y, color=:gray, linestyle=:dash, label="")
annotate!(p, [(45, -0.25*100, text("Bertrand", :red, 14, :left))])
annotate!(p, [(42, 0.06*100, text("Cournot", :blue, 14, :left))])
annotate!(p, [(86.0, -0.025*100, text(L"S (\%)", 10, :left))])
annotate!(p, [(0, -0.35*100, text(L"$\frac{W- W_0}{W_0} \times 100\% $", 10, :left))])
...
annotate!(p, [(30, -0.05*100, text(C_shr_text, 7, :left, color=:gray))])
annotate!(p, [(30, -0.08*100, text(B_shr_text, 7, :left, color=:gray))])
savefig(p, "Figures/C2N1_delta_Wbigger_revised1.pdf")
...
annotate!(p, [(-55, -0.10*100, text("Bertrand", :red, 14, :left))])
annotate!(p, [(22, 0.04*100, text("Cournot", :blue, 14, :left))])
annotate!(p, [(86.0, -0.03*100, text(L"S (\%)", 10, :left))])
annotate!(p, [(0, -0.35*100, text(L"$\frac{W- W_0}{W_0} \times 100\% $", 10, :left))])
...
C_shr_text = "Cournot mkt shrs w/o S: " * string(round.(C_shr_list, digits=3))
B_shr_text = "Bertrand mkt shrs w/o S: " * string(round.(B_shr_list, digits=3))
annotate!(p, [(30, -0.15*100, text(C_shr_text, 7, :left, color=:gray))])
annotate!(p, [(30, -0.18*100, text(B_shr_text, 7, :left, color=:gray))])
savefig(p, "Figures/C2N1_delta_Wsmaller_revised1.pdf")
