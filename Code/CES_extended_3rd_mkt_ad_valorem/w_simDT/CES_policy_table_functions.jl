...
function W_hausman_inferred_true_varyM_full_f(InputDTFile::String, InputEstFile::String, critical_value::Float64, c::Int64, selected_M::Int64; Output = false, OutputFile::String = "")
    ...
    # mean(dfB.mean_RS0)
    # mean(dfC.mean_RS0)
    # maximum(dfB.mean_RS0)
    # maximum(dfC.mean_RS0)
    # minimum(dfB.mean_RS0)
    # minimum(dfC.mean_RS0)
   ...
   #  length(eqbm_t_list_C[1].eqbm_m_list)
    W_dfB = []
    W_dfC = []
    for t in 1:T_length_B
        ...
        # est_eqbm_true_R_t = estimated_eqbm_t_f("B", eqbm_t_list_B[t], dfB_t.σDSB_GMM[1])
        ...
        if inferred_conduct_t == "N"
            W_df_m = optimal_s_inferred_true_full_t("B", "N", eqbm_t_list_B[t], eqbm_t_list_B[t], c, GP.C, selected_M, T_dict_B[t])
            push!(W_dfB, W_df_m)
        end   
    end
    for t in 1:T_length_C
        ...
        if inferred_conduct_t == "N"
            W_df_m = optimal_s_inferred_true_full_t("C", "N", eqbm_t_list_C[t], eqbm_t_list_C[t], c, GP.C, selected_M, T_dict_C[t])
            push!(W_dfC, W_df_m)
        end
    end
    W_dfB = vcat(W_dfB...)
